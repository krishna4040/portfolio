...
const Contact = () => {
    ...
    const sumbitHandler = (data) => {
        ...
        try {
            const response = axios.post('http://localhost:5000/api/createEntry',{
                name,
                email,
                subject,
                message,
        ...
        <section id='contact' className='w-full bg-[#e7e7e7] -mt-4 p-10'>
            <div>
                ...
                <form noValidate onSubmit={handleSubmit(sumbitHandler)} className='flex flex-col justify-center w-full gap-10 mx-20 my-8 lg:gap-0'>
                    <input className='w-full -ml-20 lg:ml-7 h-10 py-0 text-lg font-medium shadow-md lg:w-[1200px] lg:px-8 lg:m-7' type="text" placeholder='Enter Your Name' {...register("name",{required:true})} />
                    <input className='w-full -ml-20 lg:ml-7 h-10 py-0 text-lg font-medium shadow-md lg:w-[1200px] lg:px-8 lg:m-7' type="email" placeholder='Enter Your Email' {...register("email",{required:true})} />
                    <input className='w-full -ml-20 lg:ml-7 h-10 py-0 text-lg font-medium shadow-md lg:w-[1200px] lg:px-8 lg:m-7' type="text" placeholder='Enter Your Subject' {...register("subject",{required:true})} />
                    <textarea className='w-full -ml-20 lg:ml-7 h-auto px-8 py-0 pt-4 text-lg font-medium shadow-md lg:w-[1200px] lg:m-7' cols="30" rows="10" placeholder='Enter Your Message' {...register("message",{required:true})}></textarea>
